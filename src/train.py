...
class ModelHandler:
    def __init__(self, model, stratify=True, preprocessor=None, n_splits=5):
        ...
        :param stratify: Si True, utilise une stratification dans la
            validation croisée
        :param preprocessor: Transformations de prétraitement
...
if __name__ == "__main__":
    ...
    df["a_quitte_l_entreprise"] = df["a_quitte_l_entreprise"].apply(
        {"Non": 0, "Oui": 1}.get
    )
    # print (df)
    ...
    X_train = X_train.drop(
        columns=[col for col in excluded_cols if col in X_train.columns],
        errors="ignore",
    )
    ...
    try:
        ARTIFACT_PATH = (
            Path(__file__).parent.parent / "model" / "ml_model.joblib"
        )
